...
class DeformConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        if isinstance(k, tuple):
            k = k[0] if k[0] == k[1] else k
        self.conv = DeformConv2d(c1, c2, k, s, autopad(k, p, d), bias=False)  # type: ignore
...
class DeformConv2d(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, d=1, bias=None, modulation=True):
        """
        Args:
        ...
        self.padding = p
        self.stride = s
        self.zero_padding = nn.ZeroPad2d(p)
        self.conv = nn.Conv2d(
            c1, c2, kernel_size=k, stride=k, bias=bias  # type: ignore
        )

        self.p_conv = nn.Conv2d(c1, 2 * k * k, kernel_size=3, padding=1, stride=s)
        nn.init.constant_(self.p_conv.weight, 0)

        self.modulation = modulation
        if modulation:
            self.m_conv = nn.Conv2d(c1, k * k, kernel_size=3, padding=1, stride=s)
    ...
    def forward(self, x):
        ...
        # modulation
        if self.modulation:
            m = m.contiguous().permute(0, 2, 3, 1)  # type: ignore
            m = m.unsqueeze(dim=1)
